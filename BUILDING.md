...
This repository depends on SDL3, which is built from source as an ordinary part of this project.
CMake expects SDL3 to be available in a sibling `thirdparty` directory, so that it can pull it in via `add_subdirectory` (not `find_package`).
...
The SDL3 DLL is copied next to the executable automatically as part of the build.
